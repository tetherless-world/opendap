...
version="$1"
if [[ -z "$version" ]]; then
    version="`date +%Y%m%d`-`date +%s`-`resource-name.sh | awk '{print substr($0,1,4)}'`"
fi
...
if [[ ! -d $version || ! -d $version/source || `find $version -empty -type d -name source` ]]; then
   ...
   pushd $version &> /dev/null
      versionD=`cr-dataset-uri.sh --uri`
   popd &> /dev/null
   ...
   pushd $version/source &> /dev/null
      ...
      cr-default-prefixes.sh --turtle > opendap-provenance.ttl
      echo                           >> opendap-provenance.ttl
      echo "@base <$versionD/>."     >> opendap-provenance.ttl
      echo                           >> opendap-provenance.ttl
      echo "<a1> a prov:Activity ."  >> opendap-provenance.ttl
